...
O=SCRATCH/'regen44'; O.mkdir(exist_ok=True)
agy=json.load(open(SCRATCH/'tier44'/'gemini-tier44.json'))
t4=[(int(L),int(b)) for L in sorted(agy,key=int) for b in sorted(agy[L],key=int) if agy[L][b]['tier'] in (3,4) and int(L) not in (40,41,42,43)]
print('birds',t4); out=[]
...
def one(n,i):
    ...
    for model in ('openai/gpt-image-2.5-sunburst',):
        tag=model.split('-')[-1]; t0=time.time()
        try: flat=Image.open(O/f'L{n}_{i}_{tag}_flat.png').convert('RGB') if (O/f'L{n}_{i}_{tag}_flat.png').exists() else edit_image(crop,PROMPT,model=model,quality='low')
        ...
        rec=dict(level=n,bird=i,tier=agy[str(n)][str(i)]['tier'],model=tag,seconds=round(dt,1),flat_ok=bool(ok),reason=str(reason),flat_size=flat.size,crop_size=crop.size)
        if bb:
            cut=cut.crop(bb); cut.save(O/f'L{n}_{i}_{tag}_cutout.png')
            fit=fit_sprite_to_painted(cut,crop,None); rec['fit']=fit
        out.append(rec); print(rec['level'],rec['bird'],rec['tier'],rec['fit'] and rec['fit']['score'],rec['fit'] and rec['fit']['pop'],flush=True)
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as ex: list(ex.map(lambda t: one(*t), t4))
