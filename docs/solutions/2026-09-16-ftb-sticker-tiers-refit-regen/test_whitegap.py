"""Guard tests for whitegap.py: known gaps are found (and only where the legs are), known white plumage is left alone."""
from pathlib import Path
import pytest
from whitegap import find_gaps, punch_gaps, load

FX = Path(__file__).parent / 'whitegap_fixtures'
...
def test_punch_only_touches_the_gap(path):
    a = load(path); gaps = find_gaps(a); b, px = punch_gaps(a, gaps)
    assert px <= gaps[0]['size'] * 3
    changed = (a[..., 3] != b[..., 3])
    assert changed.sum() == px
    assert find_gaps(b) == []                       # idempotent: nothing left to find
    # everything outside the punched area is byte-identical
    assert (a[~changed] == b[~changed]).all()
