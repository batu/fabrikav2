...
REPO='/Users/base/dev/appletolye/fabrikav2'; O=SCRATCH/'regen3col'; O.mkdir(exist_ok=True); T=320; assets=[]
...
for n in range(1,9):
    ...
    for i,d in enumerate(level['dogs']):
        sp=d['sprite']
        if not sp.get('technique','').startswith('gpt-image-2.5'): continue
        ...
        tiles=[('PAINTED (in scene)',crop),(f'ORIGINAL sprite  (was T{old_t[str(n)][str(i)]["tier"]})',on_grey(old,ox,oy)),(f'NEW sprite  (agy T{agy[str(n)][str(i)]["tier"]})',on_grey(new,nx,ny))]
        ...
        dr.text((8,8),f"Level {n}  bird #{i}     {agy[str(n)][str(i)]['why'][:70]}",font=F,fill=(255,255,255)); name=f'L{n:02d}_{i:02d}.jpg'; sheet.save(O/name,quality=80); assets.append(name)
...
f'<h1>All {len(assets)} regenerated birds on the phone build 9e23306bb: painted scene | original sprite | new sprite</h1><p>Same crop for all three columns. Sprites are drawn on grey at the exact position and size the game uses (original from the shipped level.json, new from the current one after refit). Tier labels: original from agy on the shipped sticker, new from agy on the regenerated cutout.</p>']
cur=None
for a in assets:
    L=int(a[1:3])
