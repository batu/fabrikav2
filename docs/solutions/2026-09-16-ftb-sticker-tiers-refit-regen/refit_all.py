...
for n in [int(x) for x in sys.argv[1:]]:
    ...
    for i,d in enumerate(level['dogs']):
        ...
        if not f: rows.append(dict(level=n,bird=i,status='skipped: no fit')); continue
        ...
        rows.append(dict(level=n,bird=i,status=status,scale=f['scale'],ratio=f['ratio'],pop=f['pop'],dx=int(rx-x),dy=int(ry-y)))
