...
def judge(k, panels, tiers_path, only=None):
    ...
    sh('tier_openrouter.py',panels,tiers_path,'--all-none')
...
for k in sys.argv[1:]:
    ...
    regen=sorted(regen_j|refused,key=int)
    summary=dict(id=k,birds=n,judge_regen=sorted(regen_j,key=int),refit_refused=sorted(refused,key=int),regen=regen)
    if regen:
        sh('intake_regen.py',S/'tiers.json',S/'refit1.json',k,REGEN_OUT=S/'regen.json')
