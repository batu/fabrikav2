...
- Production validators fail closed for absent/malformed selected providers, dev-shell leakage, signing identity, and AAB identity.
...
|------|--------------------|--------|
| test | `npm run test:unit -w @fabrikav2/find_the_dog` | passed, 282 tests |
| test | scoped Find the Bird unit run excluding sparse-content suites | passed, 344 tests |
| test | `npm run test:unit -w @fabrikav2/sdk` | passed, 337 tests |
| test | native-shell, game-env, and Google Play focused suites | passed, 12 + 39 + 6 tests |
...
The tests directly exercise provider composition and deterministic release validators. Both Android environment validators accepted complete synthetic, non-owner test values before the builds reached missing sparse content. This does not prove Gradle compilation, a signed AAB, native provider initialization, dashboard receipt, purchases, ads, Crashlytics delivery, device launch, or Play availability. The exact production inputs were intentionally not fabricated.
