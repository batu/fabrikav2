 ...
 *
 * Android note: the current DRD Keymaster source row still records the publisher
 * bundle `com.baseardahan.hiddenobj`, while this checkout ships Android as
 * `com.utolye.findthedog`. Keep both values explicit so future code cannot
 * confuse source-row provenance with the APK package identity.
 */
export const KEYMASTER_PUBLIC_ROWS: Readonly<Record<KeymasterRuntimePlatform, KeymasterPublicRow>> = {
  ios: {
    ...
    runtimeBundleId: 'com.baseardahan.hiddenobj',
    appleStoreId: null,
    appLovinAdUnitIds: {
  ...
  android: {
    ...
    storeLink: null,
    keymasterBundleId: 'com.baseardahan.hiddenobj',
    runtimeBundleId: 'com.utolye.findthedog',
    appleStoreId: null,
    appLovinAdUnitIds: {
