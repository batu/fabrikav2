...
export class GameState {
  ...
  applyRewardProgressToActiveCompletion(goal: number, hintRewardAmount: number): { applied: boolean; rewardGranted: boolean; hintsGranted: number } {
    ...
    if (this._rewardProgressCount >= safeGoal) {
      ...
      // yields 0; hold the accumulated count (>= goal) until there is
      // room for this ordinary free grant. Rewarded ads settle separately
      // without this balance cap. Without this guard the count
