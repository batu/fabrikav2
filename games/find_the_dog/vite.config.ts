...
import { nativePublicBundlePlugin } from './src/build/nativePublicBundle.ts';
...
const admobStub = path.join(gameRoot, 'src/sdk/shims/capacitor-community-admob.ts');
...
function environmentKeysForMode(mode: string): string[] {
  return FIND_THE_DOG_ENV_KEYS.filter((key) => {
    ...
    if ((mode === 'ios' || mode === 'android') && key === 'VITE_FTD_AD_LIFECYCLE_DRIVE') {
      return process.env.VITE_ENABLE_TEST_HARNESS === 'true' && process.env.VITE_FTD_AD_LIFECYCLE_DRIVE === 'true';
    }
    return true;
  });
}
