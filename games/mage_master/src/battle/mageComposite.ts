 ...
 *
 * The canvas is taller than the body box: the staff reaches past the head, so
 * COMPOSITE_HEADROOM transparent rows sit above the figure. The body keeps the
 * bottom COMPOSITE_SIZE square with the feet on its lower edge, so consumers
 * that want the bare figure scale against COMPOSITE_SIZE, not the canvas height.
 */
...
export function composeMage(look: MageLook): Promise<HTMLCanvasElement> {
  ...
  const promise = (async () => {
    ...
    const drawStaff = (): void => {
      ...
      ctx.save();
      ctx.translate(anchor.x * COMPOSITE_SIZE, COMPOSITE_HEADROOM + anchor.y * COMPOSITE_SIZE);
    ...
    if (base) ctx.drawImage(base, 0, COMPOSITE_HEADROOM, COMPOSITE_SIZE, COMPOSITE_SIZE);
    if (garment) {
      const tint = document.createElement("canvas");
      tint.width = COMPOSITE_SIZE;
      tint.height = COMPOSITE_HEIGHT;
      const tctx = tint.getContext("2d");
      if (tctx) {
        tctx.drawImage(garment, 0, COMPOSITE_HEADROOM, COMPOSITE_SIZE, COMPOSITE_SIZE);
        tctx.globalCompositeOperation = "multiply";
        tctx.fillStyle = rarityColor(look.armorRarity);
        tctx.fillRect(0, 0, COMPOSITE_SIZE, COMPOSITE_HEIGHT);
