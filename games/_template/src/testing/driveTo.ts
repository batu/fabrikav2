 ...
 * deps to this game's real transitions — a port replaces the placeholder deps
 * there, this file itself never changes. Confirmed by
 ...
 *
 * TODO(port): the `sceneIs`/`playingReady` predicates assume the shared kernel
 * FlowMachine state names (`menu`/`playing`/`complete`/`failed`/`paused` —
 * `@fabrikav2/kernel` `FlowStates`). A port whose `snapshot().scene` uses those
 * names gets working confirms for free; otherwise adjust the predicates to
 * match its own state fingerprint.
 */
