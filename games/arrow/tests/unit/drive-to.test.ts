 ...
 * honestly reports `false` for terminal drivers that do not actually move the
 * queried snapshot into the target state.
 */
...
describe("_template driveTo — deterministic per-state navigation", () => {
  ...
  it("returns false when the terminal driver never reaches the state (confirm-before-resolve)", async () => {
    // An autoWin that never transitions must resolve `driveTo('win')` to an
    // honest `false`, not a bare `true` — the whole point of the confirm poll.
