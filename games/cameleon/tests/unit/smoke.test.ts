...
describe("Cameleon smoke", () => {
  it("boots the kernel flow machine, controller, and canvas shell", async () => {
    ...
    const boot = await bootGame(app, {
      ...
      startRuntime: false,
      query: { bodies: "white", dir: "screenprint", mode: "confirm" },
    });
    ...
    expect(app.querySelector<HTMLButtonElement>(".cameleon-screen__direction-button[data-direction='screenprint']")?.getAttribute("aria-pressed")).toBe("true");
    // single production direction: the picker row is hidden (docs/LEVELS.md §7)
    expect(app.querySelector<HTMLElement>(".cameleon-screen__direction-button")?.closest("section")?.hidden).toBe(true);
