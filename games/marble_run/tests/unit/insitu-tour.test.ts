...
function snapshotMatchesMarbleRunDriveState(state: DriveState, raw: unknown): boolean {
...
function runMarbleRunTour(harness: TourHarness): Promise<void> {
  return maybeRunInsituTour(harness, {
    snapshotMatchesState: snapshotMatchesMarbleRunDriveState,
  });
}

describe("marble_run maybeRunInsituTour — allstates", () => {
  ...
  it("drives every canonical state via driveTo and marks each confirmed state", async () => {
    ...
    const run = runMarbleRunTour(harness);
...
describe("marble_run maybeRunInsituTour — no script", () => {
  afterEach(() => {
    vi.unstubAllEnvs();
    document.body.innerHTML = "";
    setTourSearch("");
