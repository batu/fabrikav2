...
describe("marble_run bootstrap insitu tour wiring", () => {
  afterEach(() => {
  ...
  it("starts maybeRunInsituTour with the mounted harness when the test harness is enabled", async () => {
    ...
    const createMarbleRunHarness = vi.fn(() => harness);
    const snapshotMatchesMarbleRunDriveState = vi.fn();
    ...
    vi.doMock("../../src/attribution/AttributionService", () => ({
      ...
      createMarbleRunAttributionProvider: vi.fn(() => ({
    ...
    vi.doMock("../../src/testing/TestHarness", () => ({
      createMarbleRunHarness,
      snapshotMatchesMarbleRunDriveState,
    }));
