...
export function showPlayEntryTransitionCover(): void {
  ...
  `;
  // Phase-sync the backdrop's motif drift to the live home pattern so it does
  // not jump when this backdrop paints over the real home. The motif runs as a
  // pseudo-element animation, so align its clone via a negative delay.
  const motifAnim = document.getAnimations().find(
    (animation): animation is CSSAnimation => (
      animation instanceof CSSAnimation && animation.animationName === 'homePawDrift'
    ),
  );
  ...
  requestAnimationFrame(() => requestAnimationFrame(() => {
    if (
      isCurrentTransition(cover, generation, 'play-entry')
      && cover.dataset.transitionState === 'arming'
    ) {
      cover.dataset.transitionState = 'holding';
    }
...
function hidePlayEntryTransitionCover(
  cover: HTMLElement,
  generation: string = coverGeneration(cover),
): void {
...
export function hidePlayEntryTransitionCoverAfterSceneRender(scene: Phaser.Scene): void {
  ...
  const scheduleHide = (): void => {
    if (scheduled) return;
    scheduled = true;
    // Let the browser paint the board behind the home clone before revealing.
    requestAnimationFrame(() => requestAnimationFrame(() => {
      const cover = transitionRoot();
      if (cover?.dataset.transitionKind === 'play-entry') {
