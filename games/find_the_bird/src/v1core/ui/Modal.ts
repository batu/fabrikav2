import { buildV1ButtonElement, type ButtonVariant } from './Button';
...
function assertFreshSlot(el: HTMLElement): void {
  if (el.parentNode) {
    throw new Error('mountModal body/actions slots must be fresh elements without an existing parent.');
  }
}

export function mountV1Modal(opts: ModalOptions): UiHandle {
