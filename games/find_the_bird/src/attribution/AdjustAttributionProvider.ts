...
} from './AdjustAttributionPlugin';
import { AdjustAttribution } from './AdjustAttributionPlugin';
import type { AdjustIosConfig } from './AdjustConfig';
import { redactAdjustToken } from './AdjustConfig';
...
interface AdjustAttributionProviderOptions {
...
export class AdjustAttributionProvider implements AttributionProvider {
  ...
  async track(eventName: AttributionEventName, params: AttributionParams = {}): Promise<void> {
    await this.init();
    if (!this.initialized || this.permanentlyDisabled) return;
