...
// reads storage at import time (sandboxed Portal preview / private browsing).
...
export async function startAnalyticsBootstrap(
  hadExistingStateAtBootstrap: boolean,
  bootstrapStorageDurability: FirstOpenStorageDurability,
): Promise<void> {
  const storageDurability = bootstrapStorageDurability === 'volatile'
    ? 'volatile'
    : runtimeStorageDurability;
  await analytics.init({ hadExistingStateAtBootstrap, storageDurability });
  try {
    analytics.setCohortBucket(await initializeCohort());
  } catch (err: unknown) {
