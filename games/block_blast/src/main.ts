...
import { createBlockBlastHarness } from "./shell/harness.ts";

/**
 * Boot the game: stand up the kernel screen-flow machine, the deterministic
 * Block Blast flow controller, and the visible Canvas2D board shell.
 */
export function bootGame(mountInto: HTMLElement) {
  const machine = createFlowMachine();
  const controller = createBlockBlastController({
    env: import.meta.env.MODE === "test" ? "test" : "development",
  });
  const screen = mountBlockBlastScreen({ mountInto, controller });
...
if (appRoot) {
  ...
  if (TEST_HARNESS_ENABLED) {
    ...
    const harness = createBlockBlastHarness({
      buildVersion: "dev",
      packageId: "com.basegamelab.block_blast.dev",
