 ...
 * honestly reports `false` when a terminal driver lies or cannot reach its
 * target state.
 */
...
describe("block_blast driveTo — deterministic per-state navigation", () => {
  it.each([
  ...
  it("returns false when the terminal driver never reaches the state (confirm-before-resolve)", async () => {
    ...
    // to an honest `false`, not a bare `true`.
...
describe("createBlockBlastHarness driveTo", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });
  ...
  it.each([
    ...
    ["settings", "menu", true] as const,
  ])("driveTo(%s) reaches + confirms it on the Block Blast harness", async (state, scene, settingsOpen) => {
    installMemoryStorage();
    localStorage.clear();
    const harness = createBlockBlastHarness({ buildVersion: "test", packageId: "com.basegamelab.block_blast.dev" });
