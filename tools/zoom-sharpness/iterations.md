...
| 3 | WebGL1 cannot mipmap the shipped non-power-of-two level textures, so use its explicit fallback: generate one high-quality canvas-prefiltered color tier at the zoom-1 display footprint, derive a matching grayscale tier, and switch both to the source-resolution tier at the geometric sampling midpoint. | pending conductor scoring | pending conductor scoring | pending conductor scoring | pending conductor scoring | one extra in-memory canvas downscale plus grayscale generation during setup; pending measurement | source tiers remain resident and add one prefiltered color+grayscale pair; pending decoded-residency guard | deferred to plateau | PENDING — conductor runs `node tools/zoom-sharpness/eval.mjs --out /tmp/zoom-2-iteration-3` |
