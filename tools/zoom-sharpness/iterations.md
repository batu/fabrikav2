...
| 3b | Keep iteration 3's prefiltered tiers, but preserve the level-derived display width and height across each color/grayscale texture swap so resident resolution is transparent to world coordinates. | pending conductor scoring | pending conductor scoring | pending conductor scoring | pending conductor scoring | same as 3a; pending measurement | same as 3a; pending decoded-residency guard | deferred to plateau | PENDING — conductor reruns `node tools/zoom-sharpness/eval.mjs --out /tmp/zoom-2-iteration-3b` |
