...
export default function StepConfigure({
  ...
  const scales = config.scales ?? {
    none: { label: 'No scale', description: 'Keep the current prompt unchanged.', prompt: '' },
  };
  ...
  const defaultScale =
    sticky.scale && scaleKeys.includes(sticky.scale) ? sticky.scale : 'none';
  const defaultStyle =
  ...
  }, [setting, scene, style, view, scale, entity, bgModel, inpaintModel, upscaleEnabled, upscaleModel, upscaleTargetLongEdge, nDogs, upscaleModels.length, onUpscaleSettingsChange]);
  ...
  }, [style, entity, bgModel, inpaintModel, upscaleEnabled, upscaleModel, upscaleTargetLongEdge, upscaleModels, nDogs, aspectRatio, imageSize, setting, scene, view, scale, config, onGenerationStart, onGenerationCreateFailed, onSessionConfigured, startBgStream]);
  ...
  const multiSceneJobsRef = useRef(multiSceneJobs);
  multiSceneJobsRef.current = multiSceneJobs;
  const resumableJobSignature = multiSceneJobs
    .filter((job) => (
      !TERMINAL_MULTI_SCENE_STATUSES.has(job.status)
      && job.sessionId
      && job.jobId
      && multiSceneResumeKeysRef.current?.has(job.sceneKey)
    ))
    .map((job) => `${job.sceneKey}:${job.sessionId}:${job.jobId}`)
    .sort()
    .join('|');

  useEffect(() => {
    const currentJobs = multiSceneJobsRef.current;
    const unfinishedJobs = currentJobs.filter((job) => !TERMINAL_MULTI_SCENE_STATUSES.has(job.status));
    ...
    const pollJobs = async (): Promise<void> => {
      await Promise.all(resumableJobs.map(async (job) => {
        ...
        try {
          ...
          const cancelledJob = response.status === 'cancelled';
          const terminal = failed || cancelledJob || response.status === 'succeeded';
          if (response.status === 'succeeded' && latestBackground && effectiveUpscaleEnabled) {
            updateMultiSceneJob(job.sceneKey, {
              status: 'upscaling',
              jobStatus: response.status,
              succeeded: response.succeeded,
              failed: response.failed,
              backgroundFile: latestBackground.file,
              errors: [],
            });
            const controller = new AbortController();
            multiSceneUpscaleAbortRef.current.set(job.sceneKey, controller);
            try {
              const upscaled = await upscaleBackground(
                sessionId,
                latestBackground.index,
                upscaleModel,
                upscaleTargetLongEdge,
                false,
                controller.signal,
              ...
              if (!cancelled) {
                ...
                updateMultiSceneJob(job.sceneKey, {
                  status: 'ready',
                  backgroundFile: upscaled.background.file,
                });
              }
            } catch (err) {
              ...
              if (multiSceneUpscaleAbortRef.current.get(job.sceneKey) === controller) {
                multiSceneUpscaleAbortRef.current.delete(job.sceneKey);
              }
            }
            return;
          }
          ...
          updateMultiSceneJob(job.sceneKey, {
            status: terminal ? (failed || cancelledJob ? 'failed' : 'ready') : 'generating',
            jobStatus: response.status,
            succeeded: response.succeeded,
            failed: response.failed,
          ...
          console.warn('[StepConfigure] multi-scene durable job reconcile failed; retrying', err);
        }
      }));
      if (!cancelled) {
    ...
    return () => {
      ...
      for (const job of resumableJobs) {
        multiSceneUpscaleAbortRef.current.get(job.sceneKey)?.abort();
      }
    };
  }, [effectiveUpscaleEnabled, resumableJobSignature, updateMultiSceneJob, upscaleModel, upscaleTargetLongEdge]);
    ...
    new Promise((resolve) => {
      ...
      stream.addEventListener('generate_complete', () => {
        ...
        void Promise.all(readyBackgrounds.map((background) => (
          ...
          .finally(() => {
            if (multiSceneUpscaleAbortRef.current.get(sceneKey) === controller) {
      ...
      stream.onerror = () => {
        multiSceneResumeKeysRef.current?.add(sceneKey);
        updateMultiSceneJob(sceneKey, {
          status: 'generating',
  ...
  }, [multiSceneRunning, selectedSceneKeys.length, selectedManySceneEntries, closeMultiSceneStreams, config.settings, style, bgModel, inpaintModel, nDogs, aspectRatio, imageSize, entity, view, scale, effectiveUpscaleEnabled, upscaleModel, upscaleTargetLongEdge, updateMultiSceneJob, streamMultiSceneBackground]);
      ...
      {!isCollapsed && (
        <div className="step-content">
          <div className="setup-form configure-shell">
            {/* Templates: one pick sets every recipe axis (view/style/entity/
                setting/scene/models/count). The select is an action — it snaps
                back to the placeholder after applying, and manual axis edits
                afterward are free. */}
            <div className="form-group" style={{ marginBottom: 12 }}>
              <label>
                ...
                <select
                  value=""
                  onChange={(e) => {
                    const template = (config.templates ?? []).find((t) => t.id === e.target.value);
                    if (!template) return;
                    setView(template.view);
                    setViewPrompt(config.views[template.view] ?? '');
                    setStyle(template.style);
                    setStylePrompt(config.styles[template.style] ?? '');
                    setEntity(template.entity);
                    setSetting(template.setting);
                    setScene(template.scene);
                    setSelectedScenes({ [template.scene]: true });
                    setContentPrompt(config.settings[template.setting]?.scenes?.[template.scene] ?? '');
                    setScale('none');
                    setNDogs(template.nDogs ?? 20);
                    if (config.models.some((m) => m.id === template.model)) {
                      setBgModel(template.model);
                    }
                    if (inpaintModels.some((m) => m.id === template.model)) {
                      setInpaintModel(template.model);
                    }
                  }}
            ...
            {/* Scale controls scene framing independently from output resolution. */}
            <div className="form-group">
              <label>
                Scale
                <select
                ...
                >
                  {scaleKeys.map((key) => (
                    <option key={key} value={key}>{scales[key].label}</option>
                  ))}
                </select>
              </label>
              ...
              <div className={`upscale-config-panel ${effectiveUpscaleEnabled ? 'enabled' : ''}`}>
                <label className="toolbar-toggle">
                  <input
                    ...
                    disabled={upscaleModels.length === 0 || multiSceneRunning}
                ...
                <div className="upscale-config-row">
                  <div className="form-group">
                    ...
                    <select
                      ...
                      disabled={!effectiveUpscaleEnabled || upscaleModels.length === 0 || multiSceneRunning}
                    >
