...
import { ApiError, getBackgroundGenerationJob, getSession, startBackgroundGenerationJob, upscaleBackground } from './editorApi';
      ...
      const openStream = async (): Promise<void> => {
        ...
        es.onerror = () => {
          ...
          const reconcile = async (): Promise<void> => {
            if (!isCurrent() || durableJobId === null) return;
            try {
              const job = await getBackgroundGenerationJob(sessionId, durableJobId);
              if (!isCurrent()) return;
              reconcileFailures = 0;
              setStatus((current) => ({
                ...current,
              ...
              if (!TERMINAL_BACKGROUND_JOB_STATUSES.has(job.status)) {
              ...
              if (job.status === 'succeeded') {
                readyBgs.splice(0, readyBgs.length, ...job.backgrounds);
                patchSession(queryClient, sessionId, (session) => ({
                  ...session,
                  backgrounds: job.backgrounds.reduce(
                    (backgrounds, background) => upsertBackground(backgrounds, background),
                    session.backgrounds,
                  ),
                }));
                setStatus((current) => ({
              ...
              setStatus((current) => ({
                ...
                generationErrors: [
                  ...current.generationErrors,
                  job.error ?? `background generation ${job.status}`,
                ],
              }));
            } catch (err) {
              if (!isCurrent()) return;
              reconcileFailures += 1;
              const permanentClientError = err instanceof ApiError
                && err.status >= 400
                && err.status < 500
                && err.status !== 408
                && err.status !== 429;
              const recoveryExpired = Date.now() - reconcileStartedAt >= 15 * 60 * 1000;
              if (permanentClientError || recoveryExpired) {
                setStatus((current) => ({
                  ...current,
                  generating: false,
                  generationFailed: true,
                  generationJobStatus: 'reconcile_failed',
                  generationErrors: [
                    ...current.generationErrors,
                    `durable job recovery stopped: ${errorMessage(err)}; use Resume job to try again`,
                  ],
                }));
                return;
              }
              setStatus((current) => ({
                ...
                generating: true,
                generationFailed: false,
                generationJobStatus: 'reconnecting',
              }));
              console.warn('[useBgStream] durable job reconcile failed; retrying', errorMessage(err));
              pollTimerRef.current = window.setTimeout(() => {
                void reconcile();
              }, Math.min(10_000, 1000 * (2 ** Math.min(reconcileFailures - 1, 4))));
            }
          };
          void reconcile();
        };
