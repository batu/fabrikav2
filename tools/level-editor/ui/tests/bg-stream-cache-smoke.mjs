...
async function run() {
  ...
  try {
    ...
    await page.route('**/api/sessions/bg_stream_cache_seed/background-generation/jobs/bg-job-1', async (route) => {
      disconnectedJobPolls += 1;
      if (disconnectedJobPolls === 1) {
        await route.fulfill({ status: 503, json: { detail: 'gateway restarting' } });
        return;
      }
      await route.fulfill({
        json: {
          ...
          succeeded: 1,
          failed: 0,
          backgrounds: [{ index: 0, file: 'bg_00_retry.png', generationTime: 2, width: 120, height: 180 }],
          error: null,
        },
      });
    });
    ...
    await page.evaluate(() => {
      window.__bgEventSources[0].fail();
    });
    await page.waitForFunction(() => document.querySelector('[data-testid="background-job"]')?.textContent === 'bg-job-1:succeeded');
    const disconnectedErrors = await page.getByTestId('generation-errors').innerText();
    const disconnectedStatus = await page.getByTestId('generation-status').innerText();
    if (disconnectedJobPolls !== 2 || disconnectedErrors !== '' || disconnectedStatus !== 'idle:1') {
      throw new Error(
        `Succeeded durable job should reconcile after SSE closes: polls=${disconnectedJobPolls} errors=${disconnectedErrors} status=${disconnectedStatus}`,
      );
    }
