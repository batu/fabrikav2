...
const config = {
  ...
  upscaleModels: [{ id: 'fal-ai/esrgan', label: 'ESRGAN' }],
};
...
async function run() {
  ...
  try {
    ...
    await page.route('**/*', async (route) => {
      ...
      const upscaleMatch = url.pathname.match(/^\/api\/sessions\/([^/]+)\/upscale-bg$/);
      if (upscaleMatch && route.request().method() === 'POST') {
        backgroundUpscales += 1;
        await route.fulfill({
          json: {
            background: {
              index: 0,
              file: `bg_${upscaleMatch[1].replace(/_session$/, '')}_upscaled.png`,
              generationTime: 2,
              width: 2560,
              height: 3840,
            },
    ...
    await page.evaluate(() => window.__wizardEventSources.forEach((source) => source.fail()));
    ...
    if (backgroundUpscales !== 3) {
      throw new Error(`Expected each recovered background to upscale exactly once, got ${backgroundUpscales}`);
    }
    ...
    for (const scene of ['france_cafe', 'japan_garden', 'japan_market']) {
      ...
      if (!job || job.status !== 'ready' || job.backgroundFile !== `bg_${scene}_upscaled.png`) {
