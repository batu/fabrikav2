...
export function LevelView({ state, draft, level, onEdit, onPlay }: LevelViewProps): React.JSX.Element {
  ...
  const upsertOverride = <Field extends OverrideField>(field: Field, value: OverrideValues[Field]) => {
    const next: LevelOverride = { levelId: level.id, replaces: [...new Set([...(override?.replaces ?? []), field])], values: { ...(override?.values ?? {}), [field]: value } };
    ...
    <section className="level-view" aria-labelledby="level-title">
      ...
      <div className="level-workspace">
        ...
        <div className="level-controls">
          ...
          {override === undefined ? <button className="secondary-action" data-action="override" onClick={() => upsertOverride('targetRange', level.targetRange)}>Override this level</button> : (
            <fieldset className="override-fields"><legend>Target override</legend><label>Minimum<input type="number" min="1" max={range.max} step="0.5" value={range.min} onChange={(event) => upsertOverride('targetRange', { ...range, min: Number(event.target.value) })} /></label><label>Maximum<input type="number" min={range.min} max="20" step="0.5" value={range.max} onChange={(event) => upsertOverride('targetRange', { ...range, max: Number(event.target.value) })} /></label><button className="text-action" data-action="reset" onClick={reset}>Return to Journey inheritance</button></fieldset>
          )}
          ...
          <details className="advanced"><summary>Advanced level controls</summary><div className="advanced__grid">
            ...
            <label>Seed<input aria-label="Override seed" type="number" min="0" value={override?.values.seed as number | undefined ?? (level.seed.provenance === 'unknown' ? 0 : level.seed.seed)} onChange={(event) => upsertOverride('seed', Number(event.target.value))} /></label>
          </div></details>
        </div>
      </div>
    </section>
