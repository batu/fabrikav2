...
11. **Preserve human review across representation changes.** Human review is an assertion over the governed content, not its storage representation. Migrations, identity repairs, metadata normalization, and projection rebuilds must preserve an existing approval when the complete reviewed-content hashes, item count, and stable identity set are unchanged. Invalidate approval only when governed content changes, and report the exact changed artifact classes. Never make a human repeat review because of an internal representation migration. Migration tests must begin with approved data and assert that approval and user-facing review counts survive an equivalent transformation.
